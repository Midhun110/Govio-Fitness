...
exports.getProgramDayDetails = getProgramDayDetails;
exports.autoAdvanceProgramDay = autoAdvanceProgramDay;
exports.getMuscleGroupsForFocus = getMuscleGroupsForFocus;
exports.getTargetExerciseCount = getTargetExerciseCount;
...
function getDayOfWeekString(date) {
    return exports.WEEKDAY_MAP[date.getDay()];
}
function getCalendarDateForDay(dayNumber, startDateStr) {
    const start = new Date(startDateStr);
    start.setDate(start.getDate() + (dayNumber - 1));
    return start.toISOString().split('T')[0];
}
function getSplitPattern(frequency, experienceLevel) {
...
function getProgramDayDetails(dayNumber, profile) {
...
function autoAdvanceProgramDay(profile) {
    const currentDay = profile.program_day || 0;
    if (currentDay === 0 || currentDay > 100)
        return currentDay;
 ...
 */
function getTargetExerciseCount(profile, seed) {
...
function selectExercisesForMuscle(muscleGroup, pool, profile, dayOrWorkoutsCount, targetCountOverride) {
