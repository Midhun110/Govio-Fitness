...
exports.enrichExercise = enrichExercise;
exports.getIsolatedLibraryForUser = getIsolatedLibraryForUser;
exports.getExercisesForClass = getExercisesForClass;
/**
 * Helper to identify user class based on their profile data
 */
function getUserClass(profile) {
    if (!profile)
        return 'Gym Intermediate'; // Default fallback
    // 1. Resolve Age Group
    ...
    if (loc === 'Gym') {
        ...
        if (exp === 'Beginner') {
            if (equip === 'No Equipment')
                return 'Home Beginner (No Equipment)';
 ...
 */
function getIsolatedLibraryForUser(exercises, profile) {
    if (!profile)
        return exercises.filter(ex => !ex.id.startsWith('ex-h-')); // Default to gym (no ex-h-)
 ...
 */
function getExercisesForClass(userClass, exercises, profile) {
    ...
    return enriched.filter((ex) => {
        ...
        if (userClass.startsWith('Gym')) {
            if (!meta.environments.includes('Gym'))
                return false;
